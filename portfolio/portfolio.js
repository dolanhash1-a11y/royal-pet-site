  ...
  async function load(){
    try{
      ...
      root.innerHTML=albums.map((a,ai)=>{
        ...
        return `<article class="portfolio-album"><div class="portfolio-album-head">${cover?`<div class="portfolio-album-cover"><img src="${esc(src(cover))}" alt="${esc(a.title)}" loading="lazy"></div>`:''}<div><p class="eyebrow">Альбом ${ai+1}</p><h2>${esc(a.title||'Без назви')}</h2>${a.description?`<p>${esc(a.description)}</p>`:''}<span class="portfolio-count">${items.length} ${items.length===1?'робота':items.length<5?'роботи':'робіт'}</span></div></div><div class="portfolio-grid">${items.map((x,ii)=>{
          const type=x.type==='video'?'video':(x.video_url&&!x.image?'video':'image');
          const mediaUrl=x.image||x.video_url||'';
          if(!mediaUrl)return '';
          return `<button class="portfolio-item ${type==='video'?'is-video':''}" type="button" data-album="${ai}" data-index="${ii}" data-type="${type}" data-src="${esc(src(mediaUrl))}" data-title="${esc(x.title||'Royal Pet')}" data-desc="${esc(x.description||'')}">${type==='video'?`<video src="${esc(src(mediaUrl))}" muted playsinline preload="metadata"></video><span class="portfolio-play">▶</span>`:`<img src="${esc(src(mediaUrl))}" alt="${esc(x.title||a.title||'Royal Pet')}" loading="lazy">`}${x.title?`<span class="portfolio-item-title">${esc(x.title)}</span>`:''}</button>`
        }).join('')}</div></article>`;
      }).join('');
      galleryItems=[...root.querySelectorAll('.portfolio-item')];
      galleryItems.forEach((b,i)=>b.addEventListener('click',()=>open(i)));

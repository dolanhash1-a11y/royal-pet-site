...
async function getConfig(){
 ...
 try{
  const r=await fetch(API+'/hours?booking_config='+Date.now(),{credentials:'include',cache:'no-store'});
  if(r.ok){
   const d=await r.json();
   if(d&&d[STORAGE_KEY]){
    const saved=typeof d[STORAGE_KEY]==='string'?JSON.parse(d[STORAGE_KEY]):d[STORAGE_KEY];
    if(saved&&typeof saved==='object'){
     if(saved.breeds&&typeof saved.breeds==='object'){
      cfg.breeds={...cfg.breeds};
      for(const kind of ['dog','cat']){
       if(Array.isArray(saved.breeds[kind])) cfg.breeds[kind]=saved.breeds[kind].map(x=>Array.isArray(x)?[String(x[0]??'')]:[String(x??'')]).filter(x=>x[0].trim());
      }
     }
     if(Array.isArray(saved.services)) cfg.services=saved.services.map(x=>({id:String(x?.id||slug(x?.title||'service')),title:String(x?.title||'')})).filter(x=>x.title.trim());
    }
   }
  }
 }catch{}
...
async function render(){
 ...
 el.innerHTML=`<div class="bs-head"><div><span class="gold-label">НАЛАШТУВАННЯ ЗАПИСУ</span><h2>Породи та послуги</h2><p>Тут можна змінювати лише назви, які відображаються у формі «Записатися на грумінг».</p></div><button class="small-btn gold bs-save" id="bs-save">Зберегти зміни</button></div>
 ...
 <div class="bs-card"><div class="bs-section-title"><div><h3>✂️ Послуги для запису</h3><div class="bs-muted">Редагуйте лише назву послуги, яка відображається у формі запису.</div></div><button class="small-btn" id="bs-add-service">+ Додати</button></div><div id="bs-service-list"></div></div>
 ...
 const renderServices=()=>{const list=el.querySelector('#bs-service-list');list.innerHTML=cfg.services.length?cfg.services.map((x,i)=>`<div class="bs-row" data-i="${i}">${field(x.title)}<div class="bs-actions"><button type="button" class="small-btn bs-up">↑</button><button type="button" class="small-btn bs-down">↓</button><button type="button" class="small-btn" data-remove="1">×</button></div></div>`).join(''):'<div class="bs-empty">Список послуг порожній.</div>';
  list.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{cfg.services.splice(Number(b.closest('.bs-row').dataset.i),1);renderServices()});
  list.querySelectorAll('.bs-up').forEach(b=>b.onclick=()=>{const i=Number(b.closest('.bs-row').dataset.i);if(i>0){[cfg.services[i-1],cfg.services[i]]=[cfg.services[i],cfg.services[i-1]];renderServices()}});
  list.querySelectorAll('.bs-down').forEach(b=>b.onclick=()=>{const i=Number(b.closest('.bs-row').dataset.i);if(i<cfg.services.length-1){[cfg.services[i+1],cfg.services[i]]=[cfg.services[i],cfg.services[i+1]];renderServices()}});
 };
 ...
 el.querySelector('#bs-add-dog').onclick=()=>addBreed('dog');el.querySelector('#bs-add-cat').onclick=()=>addBreed('cat');
 el.querySelector('#bs-add-service').onclick=()=>{cfg.services.push({id:slug('new-service-'+Date.now()),title:'Нова послуга'});renderServices();setTimeout(()=>{const input=el.querySelector('#bs-service-list .bs-row:last-child .bs-name');input?.focus();input?.select()},0)};
 renderBreeds('dog');renderBreeds('cat');renderServices();
 el.addEventListener('input',e=>{const row=e.target.closest('.bs-row');if(!row)return;const i=Number(row.dataset.i);if(row.closest('#bs-dog-list'))cfg.breeds.dog[i]=[row.querySelector('.bs-name').value];else if(row.closest('#bs-cat-list'))cfg.breeds.cat[i]=[row.querySelector('.bs-name').value];else if(row.closest('#bs-service-list'))cfg.services[i].title=row.querySelector('.bs-name').value;});
 el.querySelector('#bs-save').onclick=async()=>{const save=el.querySelector('#bs-save');save.disabled=true;save.textContent='Зберігаємо…';try{const uniq=new Set();for(const s of cfg.services){s.title=String(s.title||'').trim();s.id=String(s.id||slug(s.title));let id=s.id;let n=2;while(uniq.has(id)){id=`${s.id}-${n++}`}s.id=id;uniq.add(id)}for(const kind of ['dog','cat'])cfg.breeds[kind]=cfg.breeds[kind].map(x=>[String(Array.isArray(x)?x[0]:x).trim()]).filter(x=>x[0]);cfg.services=cfg.services.filter(x=>x.title);const payload={breeds:cfg.breeds,services:cfg.services};const r=await fetch(API+'/hours',{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({[STORAGE_KEY]:JSON.stringify(payload)})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Не вдалося зберегти налаштування');el.querySelector('#bs-msg').textContent='Зміни збережено ✓'}catch(e){alert(e.message)}finally{save.disabled=false;save.textContent='Зберегти зміни'}};
}

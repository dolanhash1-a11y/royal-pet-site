(()=>{
  ...
  function addView(){
    ...
    if(main&&!document.getElementById('view-portfolio'))main.insertAdjacentHTML('beforeend','<div id="view-portfolio" class="view hidden"></div>');
  }
  function openCms(){window.open('../admin/#/collections/portfolio','_blank','noopener')}
  async function show(){
    document.querySelectorAll('.view').forEach(x=>x.classList.add('hidden'));
    const root=document.getElementById('view-portfolio');if(!root)return;
    root.classList.remove('hidden');
    document.querySelectorAll('.nav-item').forEach(x=>x.classList.toggle('active',x.dataset.view==='portfolio'));
    const title=document.getElementById('page-title');if(title)title.textContent='Портфоліо';
    root.innerHTML='<div class="portfolio-admin-head"><div><h3>Портфоліо</h3><p>Керуйте альбомами, фото та відео робіт через медіаредактор.</p></div><div class="portfolio-admin-actions"><a class="small-btn" href="../portfolio/" target="_blank" rel="noopener">Відкрити сторінку</a><button class="small-btn gold" id="open-portfolio-editor" type="button">＋ Додати фото / відео</button></div></div><div class="card portfolio-admin-note"><b>Як додавати роботи</b><span>Натисніть «Додати фото / відео» — відкриється редактор медіа Royal Pet, де можна завантажити файли, створити альбом і зберегти зміни.</span></div><div id="portfolio-admin-list" class="portfolio-admin-list">Завантаження…</div>';
    ...
    try{
      ...
      if(!albums.length&&Array.isArray(d.items)){const map=new Map();d.items.forEach(x=>{const k=x.album||'Наші роботи';if(!map.has(k))map.set(k,[]);map.get(k).push(x)});albums=[...map.entries()].map(([title,items])=>({title,items}))}

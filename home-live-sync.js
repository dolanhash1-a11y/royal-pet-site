...
const local=()=>{try{const h=JSON.parse(localStorage.getItem(KEY)||'null');return h&&typeof h==='object'?h:null}catch{return null}};
async function loadFallback(){
 const [site,home,about]=await Promise.all([
  fetch(`content/site.json?v=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({})),
  fetch(`content/home.json?v=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({})),
  fetch(`content/about.json?v=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({}))
 ]);
 return {...site,...home,...about,benefits:about.benefits||[]};
}
async function getFresh(){
 if(!API)throw Error('API not configured');
 const r=await fetch(API+'/public/hours?home='+Date.now(),{cache:'no-store'});
 if(!r.ok)throw Error(`API ${r.status}`);
 const d=await r.json();
 const raw=d?.__home__;
 if(!raw)throw Error('home data missing');
 const h=typeof raw==='string'?JSON.parse(raw):raw;
 if(!h||typeof h!=='object')throw Error('invalid home data');
 return h;
}
function apply(h){
 if(!h)return;
 ...
 document.title=`${h.name||'Royal Pet'} — грумінг-студія для собак та котиків`;
 ...
 set('[data-home-title]',h.title);set('[data-home-subtitle]',h.subtitle);set('[data-home-description]',h.description);set('[data-home-button]',h.button_text);attr('[data-home-button]','href',h.button_link||'booking/');
 const sec=document.querySelector('[data-home-secondary-button]');if(sec){sec.textContent=h.secondary_button_text?`${h.secondary_button_text} →`:'Переглянути послуги →';sec.href=h.secondary_button_link||'#services'}
 ...
 if(box){let hero=box.querySelector('img');if(h.hero_image){if(!hero){hero=document.createElement('img');hero.decoding='async';box.innerHTML='';box.appendChild(hero)}hero.hidden=false;hero.src=h.hero_image+(h.hero_image.includes('?')?'&':'?')+'v='+Date.now();hero.alt=h.hero_alt||h.name||'Royal Pet'}else if(hero){hero.remove();box.innerHTML='<div class="hero-photo-placeholder" aria-label="Головне фото Royal Pet"></div>'}}
}
async function run(){
 let h=null;
 try{h=await getFresh()}catch(e){console.warn('[Royal Pet] live home unavailable',e);h=local()||await loadFallback()}
 apply(h);
}

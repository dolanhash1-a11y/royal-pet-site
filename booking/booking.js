(() => {
    ...
    servicesRoot.innerHTML = visible.length
      ? visible.map(service => `
          <label class="service-option">
            ...
            <span><b>${esc(service.title)}</b><small>${petType === 'cat' ? 'Royal Pet · догляд за котиком' : 'Royal Pet · догляд за улюбленцем'}</small></span>
            <i>›</i>
          </label>`).join('')
  ...
  function renderTimeSlots() {
    ...
    for (let m = start; m < end; m += 30) {
      ...
      timeSelect.appendChild(o); count++;
    }
  ...
  async function submitBooking(e) {
    ...
    const dt = new Date(`${dateInput.value}T${timeSelect.value}:00`);
    ...
    try {
      ...
      setMessage(formMessage, `Не вдалося надіслати заявку: ${err.message}`, 'error');
    } finally {

(()=>{
  ...
  async function render(){
    const pets=document.getElementById('profile-pets');if(!pets||!window.customerAuth?.me||!window.customerAuth?.appointments)return;let user=null,items=[];try{user=await window.customerAuth.me();if(user)items=await window.customerAuth.appointments()}catch{return}if(!user)return;reviewNotice(items);

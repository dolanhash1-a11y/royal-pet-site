(()=>{
  ...
  function twoHourNotice(items){
    const profile=document.getElementById('profile-pets');if(!profile)return;
    const old=document.getElementById('profile-two-hour-reminder');if(old)old.remove();
    const now=new Date(),cutoff=new Date(now.getTime()+2*60*60*1000);
    const item=(Array.isArray(items)?items:[]).filter(x=>{const d=new Date(x.preferred_time);return !isNaN(d)&&d>now&&d<=cutoff&&!blocked(x.status)}).sort((a,b)=>new Date(a.preferred_time)-new Date(b.preferred_time))[0];
    if(!item)return;
    const dismissed=localStorage.getItem('royal_pet_two_hour_notice_dismissed');
    const key=String(item.id||item.preferred_time);if(dismissed===key)return;
    addReviewStyles();const pet=esc(item.pet_name||'Улюбленець'),dt=getDateTime(item.preferred_time),services=esc(item.additional_services||item.services||'грумінг');
    const notice=document.createElement('section');notice.id='profile-two-hour-reminder';notice.className='review-reminder urgent';
    notice.innerHTML=`<div class="review-reminder-icon">⏰</div><div class="review-reminder-body"><strong>Грумінг вже скоро</strong><p><b>${pet}</b> записаний${dt?.time?` на <b>${dt.time}</b>`:''}. До візиту залишилось менше 2 годин. ${services ? `Послуги: ${services}.` : ''}</p><div class="review-reminder-actions"><button type="button" class="primary two-hour-view">Переглянути запис</button><button type="button" class="ghost two-hour-close">Зрозуміло</button></div></div>`;
    profile.after(notice);
    notice.querySelector('.two-hour-view')?.addEventListener('click',()=>{localStorage.setItem('royal_pet_two_hour_notice_dismissed',key);location.hash='appointments';window.dispatchEvent(new HashChangeEvent('hashchange'))});
    notice.querySelector('.two-hour-close')?.addEventListener('click',()=>{localStorage.setItem('royal_pet_two_hour_notice_dismissed',key);notice.remove()});
    if('Notification' in window&&Notification.permission==='granted'){try{new Notification('Royal Pet — нагадування',{body:`${item.pet_name||'Улюбленець'} має грумінг ${dt?.time||''}. До візиту менше 2 годин.`})}catch{}}
  }
  ...
  async function render(){
    const pets=document.getElementById('profile-pets');if(!pets||!window.customerAuth?.me||!window.customerAuth?.appointments)return;let user=null,items=[];try{user=await window.customerAuth.me();if(user)items=await window.customerAuth.appointments()}catch{return}if(!user)return;reviewNotice(items);tomorrowNotice(items);twoHourNotice(items);

(()=>{
  ...
  function groupAppointments(){
    const root=document.getElementById('appointments-list');
    if(!root)return;
    const cards=[...root.querySelectorAll('.appointment-item')];
    if(!cards.length||root.dataset.grouped==='1')return;
    const now=new Date();
    const rows=cards.map(card=>{
      const text=card.querySelector('.appointment-time')?.textContent||'';
      const m=text.match(/(\d{4}-\d{2}-\d{2})\s*[· ]\s*(\d{1,2}):(\d{2})/);
      const dt=m?new Date(`${m[1]}T${m[2].padStart(2,'0')}:${m[3]}`):null;
      const status=card.querySelector('.appointment-top span')?.textContent||'';
      return {card,dt,active:!blocked(status)&&dt&&!Number.isNaN(dt.getTime())&&dt>=now};
    });
    const upcoming=rows.filter(x=>x.active).sort((a,b)=>a.dt-b.dt);
    const history=rows.filter(x=>!x.active).sort((a,b)=>(b.dt?.getTime()||0)-(a.dt?.getTime()||0));
    const frag=document.createDocumentFragment();
    const heading=(text,extra='')=>{const h=document.createElement('div');h.className=`history-title appointments-group-title ${extra}`;h.textContent=text;return h};
    if(upcoming.length){frag.append(heading('Найближчі записи'));upcoming.forEach(x=>frag.append(x.card))}
    if(history.length){frag.append(heading('Історія записів','appointments-history-heading'));history.forEach(x=>frag.append(x.card))}
    root.append(frag);root.dataset.grouped='1';
  }
  ...
  async function render(){
    ...
    if(!user)return;
    const now=new Date();
    const future=(Array.isArray(items)?items:[]).filter(x=>{const dt=getDateTime(x.preferred_time);if(!dt)return false;const status=String(x.status||'').toLowerCase();if(blocked(status))return false;return new Date(x.preferred_time)>now}).sort((a,b)=>String(a.preferred_time).localeCompare(String(b.preferred_time)));
  ...
  const baseProfile=window.renderCustomerProfile;
  if(typeof baseProfile==='function')window.renderCustomerProfile=async()=>{await baseProfile();setTimeout(render,40)};
  const baseAppointments=window.renderCustomerAppointments;
  if(typeof baseAppointments==='function')window.renderCustomerAppointments=async()=>{await baseAppointments();setTimeout(groupAppointments,0)};
